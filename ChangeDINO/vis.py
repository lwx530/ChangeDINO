...
'''def save_feature_map(feature_tensor, save_name, save_dir="vis_results_10_9", target_size=(256, 256)):
    ...
    # 3. 通道降维：改为跨通道取最大值 (Max Pooling)
    # 这会保留所有通道中在每个像素位置上的最强响应
    feat_max, _ = torch.max(feat, dim=1, keepdim=True)

    # 4. 尺度上采样：使用双线性插值统一调整为原始输入图像的分辨率 (256x256)
    feat_up = F.interpolate(feat_max, size=target_size, mode='bilinear', align_corners=False)
...
def main():
    ...
    weight_path = "/root/autodl-tmp/ChangeDINO/checkpoints/ESDI-41/ESDI-41_resnet34_best.pth"
